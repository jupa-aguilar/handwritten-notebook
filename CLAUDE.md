...
| `src/toc.js` | The notebook's index: the prompt that asks a run of pages where its sections start and what they are called, the parsing, the batching, `branchRows`/`visibleRows` (the flat entries read as a tree, so a branch can be folded without one ever being built), and `placeEntries` — which checks each entry against the page's own text before believing it, because the model's answer is a claim and the anchor beside it is evidence. It settles three things from where the anchor turns up: the page number, the casing of the title (`sentenceCase`, from how the page writes those words when it is not shouting), and the level — the heading a page opens with is its subject, the ones below it are its sections, which is how these notebooks are written and the only trace of it that survives into the transcription. Pure but for the model call. Unlike cards it is stored on the notebook record and rides the sync manifest — a card is graded forty times in a sitting, so putting it there would drag every page's text up to Drive forty times, while an index is built once and then left alone. The price is a whole-index last-write-wins merge, which is right for something generated and would be wrong the day the entries can be edited by hand. It is settled by its own `builtAt` (`pickToc` in `db.js`), never by the notebook's `updatedAt`: the index is the one notebook field a device may not have at all, and an absent one must never beat a built one — a phone that bookmarked a page after the desktop built the index would otherwise win the notebook record and push a manifest with no index in it, erasing it everywhere. That rule is also what heals it: whichever device still holds one republishes it. |
...
| `src/guided.js` | Guided reading: the page cut into the windows a reader would otherwise have panned to, one per line or per piece of a line too long to stand at a readable size, walked by tapping. Pure geometry over the OCR word boxes. Magnification is decided once per page — a window that changed size line to line would be the same fight with the zoom this ends — and capped so the page's long lines take at most two taps, which is what actually binds on a dense page. It also reads a table by cells, and every threshold in there was measured against real pages rather than reasoned about: a gutter against the ink's height (word spacing scales with it), a row against the low quarter of the page's line gaps (in a table most gaps *are* row separations, so the median is the thing being looked for), and a column only where **two** corridors run through three bands each. That last rule is the one holding the false positives down: one corridor that recurs is a drawing, not a table, and believing it cut a heading in half and interleaved the boxes of a diagram. All of it is the fallback for a page `layout.js` has not judged; a page carrying a verdict is walked by it exactly, an empty one included. |
| `src/layout.js` | What a page's vertical corridors mean, asked of a model once per page and kept on the page like its transcription. `guided.js` finds the corridors exactly — they are the ink's own gaps — and cannot tell a table's columns from the air around a drawing, which is where it was measurably going wrong. The model may only confirm or discard corridors the geometry already found, so a wrong answer can misread a page and never invent one, and a page with no corridors is never sent at all. Asked for lazily, the first time such a page is walked in guided reading, so the only pages paid for are the ones read that way. The verdict rides the sync manifest and survives a remote that hasn't got one (same rule as `pickToc`); a re-scan drops it, since it was judged on pixels that no longer exist. |
