...
LM Studio model. Vanilla JS + Vite, no framework, no TypeScript, no test runner.
...
There is no lint or test setup. Verification is manual: see `.claude/skills/verify` (the
`/verify` skill) for how to drive the built app in Chrome and how to exercise the Electron
IPC handlers, which need the Electron runtime (plain `node` can't host them).
...
| File | Role |
|---|---|
| `src/main.js` (~2.5k lines) | The whole UI controller: flipbook render, search + highlight overlays, OCR queue, zoom viewer, pages overview, notebook manager, export/import, ZIP writer, keyboard wiring. All app state is module-level (`pages`, `currentPage`, `currentNotebookId`). |
...
- Search is accent- and case-insensitive via `foldText()` (NFD + strip combining marks, which
  keeps indexes aligned with the original text), and a page matches when it contains *every*
  query word, not the literal phrase. `searchTokens`/`pageHasAllTokens` are shared by the
  results list, the transcript highlighter and the on-image boxes so all three agree.
